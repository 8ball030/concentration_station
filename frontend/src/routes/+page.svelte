...
// @ts-nocheck
	...
	import submitBuy from '$lib/actions/submitBuy';
	import loadCoins from '$lib/actions/getCoinsList';
	...
	let cardDummyList = [0]
	let index = 0
	let currentCoinPromise = Promise.resolve({})
	let coin = {}
  	let animate = false
	let loading = false
	...
	async function handleIntention(intentionDirection) {
		...
		if (intentionDirection === INTENTION_DIRECTIONS.LIKE) {
			...
			if (res) {
				currentCoinPromise = getCurrentCoin()
				handleSwapPosted()
			}
			loading = false
		} else {
...
{#await currentCoinPromise then currentCoinPromise}
	{setCoin(() => { coin = currentCoinPromise; })}
	<div class="success">
		{#if animate}
			<div
				transition:fly={
	...
	<div class="stack grid place-items-center mt-40">
		{#each cardDummyList as dummy (dummy)}
			<Card coin={currentCoinPromise} onbuttonTapped={handleIntention} />
		{/each}
		{#if loading}
			<div class="w-96">
